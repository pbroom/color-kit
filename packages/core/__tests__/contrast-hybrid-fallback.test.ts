...
describe('contrastRegionPaths() hybrid fallback', () => {
  ...
  ])('restores work counters for $fallbackReason fallback', async (fallbackReason) => {
    vi.resetModules();
    vi.doMock('../src/contrast/region-hybrid.js', async () => {
      const actual = await vi.importActual('../src/contrast/region-hybrid.js');
      return {
        ...actual,
        contrastRegionPathsHybrid: (
          ...
          if (trace) {
            Object.assign(trace.summary, {
              ...
              scalarEvaluationCount:
                trace.summary.scalarEvaluationCount + 13,
