...
- **2026-04-02 — Wide docs widgets use column bleed, not viewport breakout**: Wrap Sandpack/playgrounds in `ck-docs-content-bleed` with `w-full min-w-0` and a capped inner shell (`max-w-7xl`, etc.) so they use the article inner grid’s side lanes; avoid `max(100%, calc(100vw - …))` plus `left-1/2 -translate-x-1/2`, which forces near-viewport width past the text column and overlaps sidebars.
