...
- **2026-03-01 — Use ES worker format for dynamic worker imports**: If a worker module adds dynamic `import()` calls (for example wasm bootstrap loading), set Vite `worker.format` to `es` or docs/app builds can fail with code-splitting errors under default IIFE worker output.
