...
- The user prefers Lab toggle group demos to match the properties panel segmented-field sizing/styling exactly, and Toggle Button hover/pressed states to use color/background changes rather than x/y translation (off: transparent rest, faint hover, stronger pressed).
...
- `PrimitiveValueInput` should stay focused on numeric behavior (formatting, parsing, stepping, scrubbing, validation); visible invalid border/error states are opt-in, and affix/unit UI belongs in wrapper utilities rather than prefix/suffix props or blurred overlays.
