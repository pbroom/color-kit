...
- **2026-02-24 — Reject tiny-area contour closures**: When closing open contrast contours against the domain loop, compare both closure directions by polygon area and prefer the non-degenerate candidate if one collapses; shortest-path selection alone can make fills appear to disappear.
- **2026-02-24 — Avoid two-anchor contrast boundary assumptions**: Contrast iso-lines in (L,C) are often multi-branch and non-linear (including missing intersections at c=0 or near-max c), so constructing region fills from only two anchor points can cause branch flips and banding.
- **2026-02-24 — Close every open contrast contour for fills**: Region fill assembly must process all open contour paths (not just the longest) and choose closure direction using a contrast-validity probe; otherwise dual-sided valid regions collapse into single-band rendering.
- **2026-02-25 — Use reference-exclusion as fill invariant**: For contrast thresholds above 1, the reference color must never lie inside a filled contrast region; enforce this during closure candidate selection and final fill filtering to prevent inverted regions.
- **2026-02-25 — Exclusion invariants must use gamut-mapped reference**: When the requested thumb is out of gamut, evaluate fill-candidate exclusion against the mapped fallback reference (`toSrgbGamut`/`toP3Gamut`) rather than raw requested L/C; raw coordinates miss inversion cases during out-of-gamut drags.
- **2026-02-25 — Seed adaptive contrast grids with cusp anchors**: For contrast contours near sharp gamut turns, seed adaptive lightness/chroma axes with cusp + edge probes and add boundary-focused sign probes in uniform-mask cells; plain uniform seeds under-sample right-edge detail.
- **2026-02-25 — Post complex review payloads via --input JSON**: For `gh api .../pulls/<n>/reviews` with multiple inline comments, prefer a single JSON payload via `--input - <<'EOF'` to avoid zsh bracket globbing and form-encoding arrays as objects.
- **2026-02-25 — Compute adaptive cusp chroma with caller search options**: In adaptive gamut-boundary sampling, keep cusp lightness from `maxChromaForHue` but derive cusp chroma through `maxChromaAt(..., options)` so coarse `maxIterations`/`tolerance` settings cannot introduce option-bypassing cusp spikes.
- **2026-02-25 — Share gamut math across layered rasters**: When splitting `ColorPlane` from out-of-gamut overlays, keep `oklch->linear` and gamut checks in one internal utility so gradient clipping/clamping and overlay masks stay numerically aligned.
- **2026-02-25 — Preserve legacy props by consuming and mapping**: During API transitions, keep deprecated props in component signatures (so they do not leak to DOM attributes), map them to the new prop semantics, and let the new explicit prop take precedence.
- **2026-02-25 — Treat checkout-canceled matrix jobs as transient first**: If multiple GitHub Actions matrix jobs fail in `actions/checkout` with “The operation was canceled,” push a fresh commit to retrigger before investigating code-level failures; these are often run-level cancellations rather than test regressions.
