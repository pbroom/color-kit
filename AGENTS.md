...
- The user prefers Lab menu rows to use one prop-driven leading/trailing slot model: show either a shortcut or a submenu arrow in the single trailing slot, avoid empty trailing columns, and use checkmark-style on/off items rather than switch toggles or selection-check controls.
