...
- The user prefers compact Lab property controls to avoid visible labels when accessible names, placeholders, and hover/focus tooltips already identify fields; icon-only segmented options should sit side by side when space allows, and related text inputs can share one row with the primary field wider (for example 70/30).
...
- `PrimitiveValueInput` should validate all inputs on commit; visible invalid border/error states are opt-in so the default remains visually minimal.
...
- The Select/Menu Lab should share one UI3-style menu implementation; Select uses the same menu as the standalone Menu demo, with trigger content modes and `Icon + text` placement defaulting to a trailing chevron, and leading elements should additively reserve the same label offset as on/off items.
