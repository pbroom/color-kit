...
- **2026-03-21 — Use flat workspace package exports**: For local packages consumed by Vite and tsup watch builds, prefer flat `exports` entries with top-level `types`/`import`/`require` fields; nested `import.types` shapes can break dep-scan and declaration resolution.
