...
description: Submit the current effort as a stacked PR, gather review and CI signals, apply focused fixes, and stop at merge-ready without merging.
---
...
2. Gather GitHub review, comment, and CI signals.
...
```bash
gh pr view --json number,url,title,baseRefName,headRefName,headRefOid
gh pr checks
```

Keep the PR number, URL, base branch, and head SHA for all later steps.

## Step 3: Gather Signals

Collect all three sources before fixing anything:

### Sticky review bot comment

The repo already posts a sticky heuristic review comment. Inspect it first:

```bash
gh pr view --comments
```

Treat the bot as a low-cost heuristic, not the final source of truth.

### Review comments and summaries

Fetch unresolved review context:
...
When CI and the summary disagree, trust `gh run view --log-failed`.

## Step 4: Fix Actionable Findings

Only apply clear, actionable fixes:

- Review feedback that requests a code or test change.
...
- `merge-ready`: PR link, current branch, validation profile run, CI status, and any residual risks.
- `blocked`: exact blocking comments/checks, latest failed run URL, and the next smallest useful action.
